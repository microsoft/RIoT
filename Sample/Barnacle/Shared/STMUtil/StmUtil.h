...
#endif
#define logError(fmt, ...)   dbgPrint("[FAIL] %s (%s@%u) - " fmt, __func__, __FILE__, __LINE__, ##__VA_ARGS__);
#define logWarning(fmt, ...) dbgPrint("[WARN] %s (%s@%u) - " fmt, __func__, __FILE__, __LINE__, ##__VA_ARGS__);
